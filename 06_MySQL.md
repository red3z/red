...
```sql
select * from table where vendor_id = 100 limit 1000000,10
```

优化后:

```sql
select * from table where vendor_id = 100 in (select vendor_id from table vendor_id = 100 limit 1000000,10)
```



方案二: 如果ID连续的话,我们还可以select * from table where id > 1000000 limit 10,效率非常高，但是前提是ID必须连续，不能有任何对ID丢失



### 批量操作调优

夜间屏蔽, 未发送的消息积压在早晨, 一次批量插入几千条消息数据, 导致慢SQL, 解决方案是拆分为一次插入500.

同时几千条数据发送完成需要修改状态, 写的是一个in的操作, 然后update 状态 in select 这些消息id, 走的类型是个range, 大概是8秒. 后来结合数据自身情况, 结合自增主键, 查询出1w里面最大的那个值, 然后更新比这个值小的数据就行. 最后走的是index merge, 把几个索引合并起来, 优化完是3s, 提升了不少.
