...
| G1                               | G1将内存等分成块为region. 屏弃了分区思想, 物理不分代, 逻辑分代. 可根据设定的最大停顿时间, 动态调整每次回收的region数量; 清理时业务逻辑不用停; |
| ZGC                              | 几乎完全并发, STW时间短.                                     |
...
1. G1物理上不分代, 逻辑上分代
2. G1没有内存碎片问题
3. 停顿时间预测, 通过动态调控每次回收region的数量保证停顿时间
4. 减少了Full GC, 一次mixed GC可以回收年轻代和老年代.
   1. 年轻代由Eden区和Survivor区组成, eden区满的时候young gc
   2. 堆的使用率达到一个阈值, 会mixed gc
   3. G1 无法在正常回收中跟上内存分配的速度时full gc
