...
为什么要线程池? 类似于JDBC连接池的连接的创建与销毁, 线程的创建与销毁比较浪费资源, 需要操作系统切换内核态, 涉及到内存资源的分配与回收. 线程池可以固定线程的数量并且复用这些线程.
...
方法1: 实现afterExecute()方法, 当线程执行完任务的时候, 判断下阻塞队列满不满, 如果不满将持久化的任务写入阻塞队列

方法2: 自定义阻塞队列, 取任务的时候判断队列满不满, 如果不满将持久化的任务写入阻塞队列
...
1. 线程池本身可以修改参数, 这些参数配置在配置中心中, 线程池监听配置中心;
1. 阻塞队列的大小是final的, 阻塞队列需要重写



线程池监控: 

1. 定义指标: 活跃线程数, 当前线程数, 阻塞队列大小, 完成任务数, 总任务数
1. 收集指标: 方法1定时任务隔几秒采集一下; 方法2重写beforeExecute()和faterExecute(), 任务执行前后采集这些指标
1. 上报监控系统, 可视化展示
